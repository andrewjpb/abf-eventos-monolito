// /features/sponsors/components/sponsors-list.tsx
"use client"

import { useInfiniteQuery } from "@tanstack/react-query"
import { getSponsors } from "../queries/get-sponsors"
import { Button } from "@/components/ui/button"
import { Plus, Search, LucideLoaderCircle, Filter } from "lucide-react"
import Link from "next/link"
import { sponsorCreatePath, sponsorsPath } from "@/app/paths"
...
import { SponsorSearchInput } from "./sponsor-search-input"
import { SponsorStatusSelect } from "./sponsor-status-select"
...
export function SponsorsList({
  ...
  const {
    ...
    queryKey: ["sponsors", searchParams.search, searchParams.active],
    queryFn: ({ pageParam }) => getSponsors({
  ...
  // Extrair a lista de patrocinadores de todas as páginas
  const sponsors = data?.pages.flatMap(page => page.sponsors) || []
        ...
        <div className="flex flex-col sm:flex-row justify-between gap-4">
          <div className="flex-1 space-y-1">
            ...
            <p className="text-muted-foreground">
              Gerenciamento de patrocinadores e suas associações com eventos
            </p>
          </div>

          <Button asChild>
            <Link href={sponsorCreatePath()}>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm text-muted-foreground">
            {totalCount} patrocinador{totalCount !== 1 ? 'es' : ''} encontrado{totalCount !== 1 ? 's' : ''}
          </span>
        </div>
      </div>
      ...
      {!isLoading && sponsors.length === 0 ? (
        <div className="flex flex-col items-center justify-center p-8 border border-dashed rounded-lg">
          ...
          <p className="text-muted-foreground text-center">
            Nenhum patrocinador encontrado com os filtros selecionados
          </p>
          <Link href={sponsorsPath()} passHref>
            <Button variant="link">
              Limpar filtros
            </Button>
          </Link>
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {sponsors.map(sponsor => (
              <SponsorCard
                key={sponsor.id}
                sponsor={sponsor as SponsorWithEvents}
